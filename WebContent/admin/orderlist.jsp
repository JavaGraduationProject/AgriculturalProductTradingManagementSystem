    ...
    <div class="table-responsive">
      <table class="table table-striped b-t b-light">
        ...
        <c:forEach items="${pageInfo.list}" var="ordermsg" varStatus="status">
          <tr>
			<td style="color:orange;font-weight:bolder">${ordermsg.ddno}</td>
			<td>${ordermsg.member.uname}</td>
			<td>
					<c:forEach items="${ordermsg.dddetailist }" var="detail">
                      	${detail.product.productname}<br/>数量:${detail.num}<br/>
                      </c:forEach>
			</td>
			<td>${ordermsg.total}</td>
			<td>${ordermsg.fkstatus}</td>
			
			<td>${ordermsg.address.addr}<br/>${ordermsg.address.name}<br/>${ordermsg.address.tel}</td>
            <td>${ordermsg.remark}</td>
            <td>${ordermsg.shfs}</td>
            <td>${ordermsg.zffs}</td>
            ...
            <c:if test="${ordermsg.fkstatus eq '已发货' and ordermsg.shfs eq '快递配送' and ordermsg.isdd ne '抵达目的地'}">
              <a href="wlrecordadd.jsp?ordermsgid=${ordermsg.id}"  class="active" ui-toggle-class="">添加物流信息</a>
            </c:if>
            
            <c:if test="${fn:length(ordermsg.wllist) gt 0}">
            <a href="wlrecordList.do?ddno=${ordermsg.ddno}"  class="active" ui-toggle-class="">查看物流信息</a>
            </c:if>
            
            
            <c:if test="${ordermsg.fkstatus eq '交易完成'}">
			<a href="fubiorderDel.do?id=${ordermsg.id}">删除</a>
			</c:if>
